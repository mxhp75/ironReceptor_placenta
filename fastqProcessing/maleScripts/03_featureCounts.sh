...
alignOutput=${PROJROOT}/fastqProcessing/maleOutput/aligned_data   # the aligned data is here
readCounts=${PROJROOT}/fastqProcessing/maleOutput/read_counts # put the count files here
...
if [[ ! -d ${readCounts} ]]
  then
	mkdir -p ${PROJROOT}/fastqProcessing/maleOutput/read_counts
fi
...
cut -f1,7- ${readCounts}/counts.out | \
   sed 1d > ${readCounts}/20240315_ironProject_male_readCounts.txt
