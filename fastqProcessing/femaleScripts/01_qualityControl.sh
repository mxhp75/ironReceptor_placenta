...
mkdir -p ${PROJROOT}/femaleOutput/log
exec 1> ${PROJROOT}femaleOutput/log/qualitycontrol.log 2>&1
set -ex
...
mkdir -p ${PROJROOT}/femaleOutput/trim_data/ ${PROJROOT}/femaleOutput/trimQC/multiqc ${PROJROOT}/femaleOutput/rawQC/multiqc
...
RAWFQ=${PROJROOT}/female_fastq			# this is where the untrimmed, merged fastq files are
RAWQC=${PROJROOT}/femaleOutput/rawQC		# put the raw QC reports here
TRIMFQ=${PROJROOT}/femaleOutput/trim_data	# put the trimmed fastq files here
TRIMQC=${PROJROOT}/femaleOutput/trimQC		# put the trimmed QC reports here
